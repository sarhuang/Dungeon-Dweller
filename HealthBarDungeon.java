 ...
 * Name: Sarah Huang
 * 
 * Purpose: The class definition for the player's health bar in the dungeon level.
 *          It draws the health bar and calculates the player's health.
 * 
 */

import greenfoot.*;  
public class HealthBarDungeon extends Actor
{   
    ...
    public HealthBarDungeon(){ //SARAH - Draws the health bar on screen
    ...
    public void act() //SARAH - Updates health bar and follow the player's location
    {
        ...
        DungeonWorld dungeon = (DungeonWorld)world; //Casting to access the methods from DungeonWorld
        setLocation(dungeon.getPlayer().getX(), dungeon.getPlayer().getY() - 70);
    ...
    public void loseHealth(){   //SARAH - Subtracts health and triggers game over screen if health reaches zero
        World world = getWorld();
            DungeonWorld dungeon = (DungeonWorld)world; //Casting to access the methods from DungeonWorld
            setLocation(dungeon.getPlayer().getX(), dungeon.getPlayer().getY() - 70);
            if(dungeon.getPlayer().hitByEnemy()){
                health--;
            }
            if(health <= 0){
                slimeDead.scale(150, 130);
                dungeon.getPlayer().setImage(slimeDead);
                deathDelay--;   //Just so the user can see the dead sprite for longer
            }
            if(deathDelay == 0){
               getWorld().removeObject(dungeon.getPlayer());
               getWorld().removeObject(this);
            }
        }

    public int getHealth(){ //Allows DungeonWorld to access player's health
        return health;
    }
